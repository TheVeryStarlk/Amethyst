using Amethyst.Components;
using Amethyst.Components.Entities;
using Amethyst.Components.Eventing;
using Amethyst.Components.Eventing.Sources.Clients;
using Amethyst.Components.Eventing.Sources.Players;
using Amethyst.Components.Messages;
using Microsoft.Extensions.Logging;

namespace Amethyst.Example;

internal sealed class DefaultSubscriber(ILogger<DefaultSubscriber> logger, IPlayerStore playerStore) : ISubscriber
{
    private const string Icon = "data:image/png;base64,";

    public void Subscribe(IRegistry registry)
    {
        registry.For<IClient>(consumer => consumer.On<Request>((_, request, _) =>
        {
            request.Status = Status.Create("Amethyst", 47, 0, 0, Message.Create("Hello, world!"), Icon);
            return Task.CompletedTask;
        }));

        registry.For<IPlayer>(consumer => consumer.On<Sent>((player, sent, _) =>
        {
            logger.LogInformation("Broadcasting: \"{Message}\"", sent.Message);

            var message = Message
                .Create()
                .Write($"{player.Username}: ").Gray()
                .Write(sent.Message)
                .Build();

            foreach (var pair in playerStore.Players)
            {
                pair.Value.Send(message);
            }

            return Task.CompletedTask;
        }));
    }
}